    ...
    public function fetchDataProcess(Request $request)
    {
        ...
        if (config('processing.use_queue')) {
            ...
            foreach ($request->file('documents') as $file) {
                ...
                $jobs[] = new ProcessDatasheetMultipartJob(
                    ...
                    (int) $userId,
                    CloneDataProcess::class,
                );
            }
            $batch = Bus::batch($jobs)->name('clone_datasheet_process')->dispatch();
        ...
        foreach ($request->file('documents') as $file) {
            ...
            try {
                ...
                return response()->json(['message' => 'Failed to upload file', 'error' => $errorResponse], is_int($e->getCode()) && $e->getCode() >= 400 ? $e->getCode() : 400);
            }
        }
